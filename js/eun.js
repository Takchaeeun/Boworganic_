...
$(document).ready(function () {
  var products = [
    ...
    { image: "images/favicon/insta8.jpg", title: "지난번에 큰개 먹이려고 구입했는데 대형견이랑 중견 둘다 잘먹어서 또 구매했어요. 배송도 빨라요.", ID: "988r***", tag: "#대형견 #사료" },
  ];

  // write-1 이미지에 호버했을 때
  // write-1 이미지에 호버했을 때 write-2를 보여줌
  $(".insta-img")
    .on("mouseenter", "a", function () {
      $(this).find(".write-2").show();
    })
    .on("mouseleave", "a", function () {
      $(this).find(".write-2").hide();
    });
  ...
  $("#load-more").click(function () {
    ...
    additionalProducts.forEach(function (product) {
      var productHtml = `<a href="#">
          <div class="write-2">
          아이디:<span>${product.ID}</span>  <br>
          <p>${product.title}</p> <br>
          <span>${product.tag}</span></div>
          <span class="write-1">
